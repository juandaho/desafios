...
export default class CartController {
  get = async (req, res) => {
    ...
    !carts
      ? res.send({ status: 404, error: "No info available" })
  ...
  put = async (req, res) => {
    try {
      ...
      res.send("The cart ID doesn't exist or you are not sending an array");
    }
  };
  ...
  putProduct = async (req, res) => {
    try {
      ...
      if (exist === true) {
        ...
        res.send({ status: 404, payload: "Product doesn't exist in cart" });
      }
    } catch {
      res.send("The product or cart doesn't exist");
    }
  };
  ...
  delete = async (req, res) => {
    try {
      ...
      res.send("The cart doesn't exist");
    }
  };
  ...
  deleteProduct = async (req, res) => {
    try {
      ...
      if (!productExist) {
        res.send({ status: 404, payload: "Product doesn't exist" });
      } else {
        let cart = await cm.getOne(id);

        if (!cart) {
          res.send({ status: 404, payload: "Cart doesn't exist" });
        } else {
          ...
          if (idToSearch == -1) {
            res.send({
              status: 404,
              payload: "El producto no existe dentro del carrito",
            });
          } else {
